...
// Define API URL
import {
  API_DELETE_CHILD_SUPERVISOR,
  API_ENDPOINT,
  API_UPDATECHILD,
} from "@env";

export const EditChildSP: FC = () => {
  const {
    control,
    handleSubmit,
    formState: { errors },
  ...
  const handleDeleteChild = async (child_id: number) => {
    ...
    try {
      const response = await fetch(
        `${API_ENDPOINT}/${API_DELETE_CHILD_SUPERVISOR}/${supervisor_id}/${child_id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <View style={styles.Inputcontainer}>
          ...
          >
            ...
            <View style={styles.MiddleSection}>
              ...
              <TextInput
                style={styles.input}
                value={selectedDate ? selectedDate : "วันเกิดเด็ก"}
                editable={false}
              ></TextInput>
