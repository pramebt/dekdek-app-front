...
// Define API URL
import { API_ENDPOINT, API_CHILDLIST, API_DELETECHILD } from "@env";
...
export const ChildList: FC = () => {
  ...
  const fetchChildList = async () => {
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINT}/${API_CHILDLIST}/${userId}`,
        {
          method: "GET",
          headers: {
  ...
  const whenGotoChildEdit = (child: Child) => {
    navigation.navigate("childEdit", { child });
  };
  ...
  const handleDeleteChild = async (child_id: number) => {
    ...
    try {
      const response = await fetch(
        `${API_ENDPOINT}/${API_DELETECHILD}/${child_id}`,
        {
          method: "DELETE",
          headers: {
  ...
  const filteredChild = children.filter(
    (child) =>
      child.firstName.toLowerCase().includes(searchText.toLowerCase()) ||
      child.lastName.toLowerCase().includes(searchText.toLowerCase()) ||
      child.nickName.toLowerCase().includes(searchText.toLowerCase())
  );
  ...
  return (
    <SafeAreaProvider>
      ...
      >
        <Text style={styles.header}>Children List</Text>

        {/* Search Section */}
        <TextInput
          style={styles.searchInput}
          placeholder="ค้นหาเด็ก..."
                  ...
                  >
                    ...
                    <View style={styles.buttonDeleteOrEdit}>
                      {/* <Pressable
                        ...
                        onPress={() => whenGotoChildEdit(child)}
                      >
                        <Text>แก้ไข</Text>
                      </Pressable> */}
                    ...
                    <View style={styles.buttonDeleteOrEdit}>
                      ...
                      <Pressable
                        ...
                        onPress={() => {
                          ...
                          setSelectedChildName(child.firstName);
            ...
            <View style={styles.modalBackground}>
              <View style={styles.modalContainer}>
                ...
                <Text style={styles.modaltitleText}>
                  ข้อมูลของเด็กคนนี้จะถูกลบออกจากระบบ
                </Text>
                  ...
                  <Pressable
                    ...
                    onPress={() => {
                      if (selectedChildId !== null) {
                        handleDeleteChild(selectedChildId);
                        setModalVisible(false);
                      } else {
