...
// Define API URL
import {
  API_ASSESSMENT_GET_DETAIL,
  API_ASSESSMENT_NEXT_PARENT,
  API_ENDPOINT,
} from "@env";

export const AssessmentPR: FC = () => {
      ...
      const fetchAssessmentDetail = async () => {
        ...
        try {
          ...
          const response = await fetch(
            `${API_ENDPOINT}/${API_ASSESSMENT_GET_DETAIL}/${child.child_id}/${aspect}/${user_id}/${childAgeInMonths}`,
            {
              method: "GET",
              headers: {
  ...
  const fetchNextAssessment = async (
    ...
    try {
      const token = await AsyncStorage.getItem("userToken");
      const response = await fetch(
        `${API_ENDPOINT}/${API_ASSESSMENT_NEXT_PARENT}/${child_id}/${aspect}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
                ...
                <View style={styles.assessmentResult}>
                  ...
                  >
                    <View style={styles.modalBackground}>
                      <View style={styles.modalContainer}>
                        <Text style={styles.modalText}>
                          คุณต้องการยืนยันผลการประเมินใช่หรือไม่?
                        </Text>
