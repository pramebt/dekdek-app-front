...
type AssessmentChildRouteProp = RouteProp<
  { assessment: { child: Child; aspect: string } },
  "assessment"
>;
...
// Define API URL
import {
  API_ASSESSMENT_NEXT_SUPERVISOR,
  API_ENDPOINT,
  API_NOT_PASSED,
} from "@env";
...
export const AssessmentSP: FC = () => {
  ...
  const route = useRoute<AssessmentChildRouteProp>();
      ...
      const fetchAssessmentDetail = async () => {
        ...
        try {
          ...
          const response = await fetch(
            `${API_ENDPOINT}/api/assessments/assessments-get-details-supervisor/${child.child_id}/${aspect}/${supervisor_id}/${childAgeInMonths}`,
            {
              method: "GET",
              headers: {
          ...
          if (response.ok) {
            ...
            console.log(
              "supervisor_assessment_id after set:",
              data.data.supervisor_assessment_id
            );
            setTimeout(() => {
              setLoading(false);
            }, 1000); // set delay
  ...
  useEffect(() => {
    if (assessmentInsert) {
      console.log(
        "Current supervisor_assessment_id:",
  ...
  const whenGotoAspect = (child: Child) => {
  ...
  const ListHistory = (child: Child, aspect: string) => {
    ...
    navigation.navigate("listhistorysp", { child, aspect });
  };
  ...
  const fetchNextAssessment = async (
    ...
    try {
      const token = await AsyncStorage.getItem("userToken");
      const response = await fetch(
        `${API_ENDPOINT}/${API_ASSESSMENT_NEXT_SUPERVISOR}/${child_id}/${aspect}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        if (data.next_assessment.status === "passed_all") {
          ...
          setAssessmentInsert({
            ...
            supervisor_assessment_id:
              data.next_assessment.supervisor_assessment_id,
          });
          console.log("supervisor_assessment_id: ", supervisor_assessment_id);
        }
        return data;
  ...
  const NotpassAssessment = async (
    ...
    try {
      const token = await AsyncStorage.getItem("userToken");
      const response = await fetch(`${API_ENDPOINT}/${API_NOT_PASSED}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        console.error("Failed to NotpassAssessment:", response.status);
        navigation.navigate("trainingsp", { child, assessmentDetails });
      }
    } catch (error) {
      console.error("Error NotpassAssessment:", error);
      navigation.navigate("trainingsp", { child, assessmentDetails });
    } finally {
      setLoading(false);
    }
                          ...
                          <Pressable
                            ...
                            onPress={() => {
                              if (assessmentInsert) {
                                ...
                                console.log(
                                  "Calling fetchNextAssessment with aspect:",
                                  aspect
                                );
                                fetchNextAssessment(
                                  child.child_id,
                                ...
                                console.log(
                                  "assessmentInsert is null or undefined"
                                );
                              }
                            }}
                          >
                        ...
                        <View style={styles.modalButtonContainer}>
                          ...
                          <Pressable
                            ...
                            onPress={() => {
                              if (assessmentInsert) {
                                console.log(
                                  "Calling Not Passed Assessment with assessmentInsert_id:",
                                  assessmentInsert.supervisor_assessment_id
                                );
                                NotpassAssessment(
                                  assessmentInsert.supervisor_assessment_id
